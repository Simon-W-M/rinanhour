...
# we can have different types of objects such as individual variables or plots
...
# THEN group by THEN calculate THEN arrange
...
# we can also resuse functions / verbs within our pipelines
# to do multiple levels of analysis in a very readable way
...
ae_attendances |> 
  ...
  summarise(mean_average = mean(attendances),
            ...
            .by = c(org_code, type)) |>
  arrange(mean_average) |>
  filter(mean_average > 3000)

# a quick note on vectorisation
# r dataframes are vectorised as default
# this makes funning complex functions across your dataframes really easy
...
# also note R doesn't like numerics as column names but will accept them
# it puts such things within `` back ticks
# this also allows for sapces in column names such as `column name`
# not super good practice but useful when importng messy excel data especially
...
# fmt_auto() also tidies up some of the number formats to be more accessible
...
#  Lets have another quick look at our Barts data

view(barts)

# We can see that Barts had three A&E types with difference numbers.
...
# lets just try that again with a facet
# lets remove the filter and bring back all types
sub_set_two <- barts %>%
  filter(period < as.Date("2018-04-01"))
...
# not the prettiest SPC and there are loads of options to customise it
# just wanted to show in a couple of lines he functionality

# R is a statsical language and has lots of lovely stats built in 

# lets see if there is a linear correlation between attendances and admissions
data_lm <- ae_attendances |>
  filter(org_code == 'RA4',
         type == '1')

# this is the line of code in base R to create a linear model
mod <- lm(admissions ~ attendances, data = data_lm)
mod

# we can then call the summary function to give us the detail
summary(mod)

# we can also use the ggstats package to produce a lovely stats graph
ggscatterstats(data_lm,
               x = attendances,
               y = admissions)

# not going to pretend to know all that this has spat out, again just showing 
# what can be easily done with a couple lines of code